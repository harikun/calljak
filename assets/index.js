   ...
   if(incomeYear < 54000000) {
    ...
    //   0 - 60 juta
     if (incomeYear > 54000000 && incomeYear <= 60000000) {
        alert[0].className = 'alert alert-success'
         ...
         Beban pajak yang ditanggung per tahun sebesar <b> Rp${tax60.toLocaleString('id')} </b>
         Dengan perhitungan:
         <b class="text-end"> 5% x Rp${Number(incomeYear).toLocaleString('id')} = Rp${tax60.toLocaleString('id')}</b>
         `;
    ...
    //   60 - 250juta
        ...
        alert[0].className = 'alert alert-success'
         ...
         Beban pajak yang ditanggung per tahun sebesar <b> Rp${total.toLocaleString('id')} </b>
         Dengan perhitungan:
         <p class="text-end"> 5% x Rp60.000.000 = Rp${tax60.toLocaleString('id')}</p>
         <p class="text-end">15% x Rp${income60to250.toLocaleString('id')} = Rp${tax60to250.toLocaleString('id')}</p>
     ...
     // 250 - 500 juta
        ...
        const total = tax60 + tax60to250 + tax250to500;
         ...
         Beban pajak yang ditanggung per tahun sebesar <b> Rp${total.toLocaleString('id')} </b> <br>
         Dengan perhitungan:
         <p class="text-end">5% x Rp60.000.000 = Rp${tax60.toLocaleString('id')}</p>
    ...
    // > 5 M
        ...
        const total = tax60 + tax60to250 + tax250to500 + tax500to5M + taxPlus5M
         ...
         <p class="text-end">25% x Rp250.000.000 = Rp${tax250to500.toLocaleString('id')}</p>
         <p class="text-end">30% x Rp4.750.000.000 = Rp${tax500to5M.toLocaleString('id')}</p>
         <p class="text-end">35% x Rp${incomePlus5M.toLocaleString('id')} = Rp${taxPlus5M.toLocaleString('id')}</p>
         <p class="text-end"><b class="text-end"> Total = Rp${total.toLocaleString('id')} </b></p>
         `;
     }
   }
}
